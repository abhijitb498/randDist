module spline_mod
    ...
    INTERFACE tridag_ser
        ...
        module procedure tridag_ser_d
    END INTERFACE

    INTERFACE tridag
        module procedure tridag_par
        module procedure tridag_par_d
    END INTERFACE

    INTERFACE locate
        module procedure locate
        module procedure locate_d
    END INTERFACE

    INTERFACE spline
        module procedure spline
        module procedure spline_d
    END INTERFACE

    INTERFACE splint
        module procedure splint
        module procedure splint_d
    END INTERFACE
...
contains 
    ...
    SUBROUTINE tridag_ser_d(a,b,c,r,u)
    USE nrtype; USE nrutil, ONLY : assert_eq,nrerror
    IMPLICIT NONE
	...
	RECURSIVE SUBROUTINE tridag_par_d(a,b,c,r,u)
	USE nrtype; USE nrutil, ONLY : assert_eq,nrerror
	!USE nr, ONLY : tridag_ser
	IMPLICIT NONE
	REAL(DP), DIMENSION(:), INTENT(IN) :: a,b,c,r
	...
	if (n < NPAR_TRIDAG) then
		...
		x = -piva(2:n2)*a(2:n-2:2)
		z = -pivc(1:nx)*c(3:n-1:2)
		call tridag_par_d(x,y,z,q,u(2:n:2))
		u(1) = (r(1)-c(1)*u(2))/b(1)
		u(3:n-1:2) = (r(3:n-1:2)-a(2:n-2:2)*u(2:n-2:2) &
			-c(3:n-1:2)*u(4:n:2))/b(3:n-1:2)
		if (nm == n2) u(n)=(r(n)-a(n-1)*u(n-1))/b(n)
	end if
	END SUBROUTINE tridag_par_d
    ...
    REAL(DP) :: a,b,h
    n=assert_eq(size(xa),size(ya),size(y2a),'splint')
    klo=max(min(locate_d(xa,x),n-1),1)
    khi=klo+1
    h=xa(khi)-xa(klo)
    if (h == 0.0) call nrerror('bad xa input in splint')
    a=(xa(khi)-x)/h
    b=(x-xa(klo))/h
    splint_d=a*ya(klo)+b*ya(khi)+((a**3-a)*y2a(klo)+(b**3-b)*y2a(khi))*(h**2)/6.0_DP
